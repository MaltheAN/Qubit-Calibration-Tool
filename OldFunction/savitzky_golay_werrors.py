...
def savgol_filter_werror(
    y, window_length, degree, error=None, cov=None, deriv=None, nPoints=None
):
    ynew = y * 0.0
    ...
    if cov is None:
        z = solve_polyfit(
            xarr, y[:window_length], degree, weight[:window_length], deriv=deriv
        )
    else:
        z = solve_leastsq(
            y[:window_length],
